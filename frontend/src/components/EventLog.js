...
const Action = (props) => <td className={style.action}>{props.children}</td>;

const Subject = (props) => (
    <td className={style.subject} width="100%">
        {props.children}
    </td>
);
...
const Song = ({ song }) => (
    <>
        <strong>{song.song.title}</strong> ⋅ {song.song.artist}
    </>
);
...
const UserJoinEvent = () => (
    <>
        <Action>
            <FontAwesomeIcon icon="door-open" className={style.upVoteColor} />
        </Action>
        <Subject />
    </>
);
...
const PlayingEvent = ({ event }) => (
    <>
        <Action>
            <FontAwesomeIcon icon={event.isPlaying ? 'play' : 'pause'} />
        </Action>
        <Subject />
    </>
);
    ...
    <>
        <Action>
            <FontAwesomeIcon
                icon="gavel"
                className={
                    event.available ? style.upVoteColor : style.downVoteColor
                }
            />
        </Action>
        <Subject>
...
const PromoteEvent = ({ event }) => (
    <>
        <Action>
            <FontAwesomeIcon
                icon="star"
                className={classnames({ [style.promoteColor]: event.promoted })}
            />
        </Action>
        <Subject>
            <Song song={event.song} />
