...
const SeekBar = ({position, duration, onChange, onUpdate}) => {
    ...
    return (
        <Slider
            ...
            <Rail>
                {({getRailProps}) => (
                    <div className={style.rail} {...(canSeek ? getRailProps() : {})}/>
                )}
            </Rail>
            <Handles>
        ...
        {(song) ? (
            <div className={style.songCard}>
                ...
                <span className={style.title}
                      title={song.song.title}>{song.song.title}</span>
                <p className={style.infoLine}>
                    ...
                    {song.song.explicit ? <span className={style.tag}
                                           title="Contains explicit lyrics">explicit</span> : ""}
                    <span className={style.tag}
                          title={`played ${song.playCount} times`}>
                        <FontAwesomeIcon icon="music"
                                         size="xs"/> {song.playCount}
                    </span>
                </p>
                <div className={style.progress}>
                    <span
                        className={style.position}>{humanDuration(player.position)}</span>
                    <SeekBar position={player.position} duration={player.duration} onChange={(e) => trackSeek(e)}/>
                    <span
                        className={style.duration}>{humanDuration(player.duration)}</span>
                </div>
            </div>
        ) : ""}
    </div>
