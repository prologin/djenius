...
async def base_test_resolver(resolver):
    ...
    # JPEG header.
    assert binascii.hexlify(cover_data.getvalue()[:64]).decode().startswith("ffd8ffe000104a4649460001")
    ...
    assert header.startswith("4f6767530002") or header.startswith("6572726f") or header.startswith("1a45") or header.startswith("0000001866747970")
